...
const makeFive = () => {
	const num = 3;
	const power = 4;
	// resultFive.textContent = num ** power;  // или так
...
//Задание 11
//Выведите на экран остаток от деления 5 на 2. Добавьте слушатель события на кнопку  "Задание 11" и выведите результат на экран.
...
const makeSeventeen = () => {
	const str = 'JavaScript';
	resultSeventeen.textContent = str.substring(7, 3);
};
...
const makeEighteen = () => {
	...
	// resultEighteen.textContent = str.replaceAll('world', 'JavaScript'); // или так
  resultEighteen.textContent = str.replace(/world/g, 'JavaScript');
};
...
const makeTwenty = () => {
	// resultTwenty.innerText = Math.max(...[10, 25, 5, 30, 15]); // или так
...
const makeTwentySeven = () => {
	resultTwentySeven.textContent = Math.round(50 + Math.random() * 50);
};
